...
class EnvOpenDogRun(gym.Env):
    ...
    def __init__(self, renders=True):
        ...
        # Motor ranges
        motorLowValues = np.array(self.numJoints * [ -0.8 ])
        motorHighValues = np.array(self.numJoints * [ 0.8 ])

        # Reduce range on coxa joints
        offset = 0

        for i in range(4):
            motorLowValues[i * 3 + offset] = -0.4
            motorHighValues[i * 3 + offset] = 0.4
        ...
        # Constants
        self.tipThresh = 0.75 # Threshold for detecting falling over
        self.tipCost = 10.0 # Cost for tipping over
        self.maxTime = 10.0 # 5 Seconds per episode
        self.timeStep = 0.02 # 50 FPS
        self.motorForce = 500
        self.motorSmoothing = 0.5
        self.motorMomentum = 0.95
        ...
        self.energyCostScalar = 0.0001 # Scale the energy cost
    ...
    def _getEnergy(self):
        ...
        for i in range(self.numJoints):
            torque = p.getJointState(self.dog, i)[3]
    ...
    def _step(self, action):
        ...
        reward = -lVel[0] - self._getEnergy() * self.energyCostScalar # Move along positive X axis and minimize energy consumption

        up = self._getUpVector()

        # If fell over
        if up[2] < self.tipThresh:
            reward = -self.tipCost # Punish for tipping over
    ...
    def _reset(self):
        ...
        self.dog = p.loadURDF("opendog.urdf", [ 0.0, 0.0, 0.36 ], globalScaling=0.1, flags=p.URDF_USE_SELF_COLLISION) # Rescaling included

        # Force friction changes
        for i in range(13):
